...
func (f *fakeEmitter) Emit(_ context.Context, pdfPath string, _ domain.ResumeData, _ domain.Profile, _ domain.Config) error {
	f.calls++
	f.lastPDF = pdfPath
	return f.err
}

func emitGenerator(render domain.Render, em usecase.ResumeEmitter) usecase.Generator {
	render.PageLimit = 1.0
	return usecase.Generator{
		Config:    &fakeConfig{cfg: domain.Config{Render: render}},
		Profiles:  &fakeProfile{p: domain.Profile{Lang: "en"}},
		Resumes:   &fakeResume{},
		Renderer:  &fakeRenderer{pages: []float64{0.5}, out: "/tmp/x.pdf"},
		Emitter:   em,
...
func TestGenerate_Emit_InvokedWhenAnyFlagSet(t *testing.T) {
	for _, r := range []domain.Render{
		{EmitMarkdown: true},
		{EmitFiltered: true},
		{EmitMarkdown: true, EmitFiltered: true},
	} {
		em := &fakeEmitter{}
		g := emitGenerator(r, em)
		out, err := g.Generate(context.Background(), usecase.GenerateInput{ProfileName: "default"})
		if err != nil {
			t.Fatal(err)
		}
		if em.calls != 1 {
			t.Errorf("render %+v: Emit called %d times, want 1", r, em.calls)
		}
		if em.lastPDF != out {
			t.Errorf("Emit got %q, want final out path %q", em.lastPDF, out)
		}
	}
}

func TestGenerate_Emit_SkippedWhenBothFlagsOff(t *testing.T) {
	em := &fakeEmitter{}
	g := emitGenerator(domain.Render{}, em)
	if _, err := g.Generate(context.Background(), usecase.GenerateInput{ProfileName: "default"}); err != nil {
		t.Fatal(err)
	}
	if em.calls != 0 {
		t.Errorf("Emit called %d times, want 0 when both flags off", em.calls)
	}
}

func TestGenerate_Emit_ErrorPropagates(t *testing.T) {
	em := &fakeEmitter{err: errors.New("disk full")}
	g := emitGenerator(domain.Render{EmitMarkdown: true}, em)
	if _, err := g.Generate(context.Background(), usecase.GenerateInput{ProfileName: "default"}); err == nil {
		t.Fatal("expected emit error to propagate")
	}
}

func TestGenerate_Emit_NilEmitterIsNoOp(t *testing.T) {
	g := emitGenerator(domain.Render{EmitMarkdown: true}, nil)
	if _, err := g.Generate(context.Background(), usecase.GenerateInput{ProfileName: "default"}); err != nil {
		t.Fatalf("nil Emitter with emit flag set should be a no-op, got %v", err)
	}
}
