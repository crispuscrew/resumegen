...
func TrimLowest(data domain.ResumeData, minElements domain.MinElements) (domain.ResumeData, bool) {
	min := minIncluded(domain.FlatNestedVisible(data))
	if min == nil {
		return data, false
	}
	min.Reason = domain.Trimmed
	data = TrimEmptyParents(data, minElements)
	return data, true
}

// TrimEmptyParents marks every included job, project, or skill category whose
// surviving children fall below minElements as Trimmed.
//
