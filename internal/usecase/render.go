...
type Generator struct {
	...
	Emitter       ResumeEmitter    // optional; nil skips LLM-ready output
	Bootstrap     Bootstrap
}
...
func (g *Generator) Generate(ctx context.Context, in GenerateInput) (string, error) {
	...
	// Scoring can leave a parent with no surviving children. Sweep before the
	// first render, otherwise a resume that already fits never reaches the
	// trim loop and ships a dangling heading.
	data = TrimEmptyParents(data, cfg.Render.MinElements)
	...
	for {
		...
		if !need {
			...
			if (cfg.Render.EmitMarkdown || cfg.Render.EmitFiltered) && g.Emitter != nil {
				if err := g.Emitter.Emit(ctx, outPath, data, profile, cfg); err != nil {
					return "", fmt.Errorf("emit outputs: %w", err)
				}
			}
			return outPath, nil
		}
