...
func (b Bootstrap) EnsureWorkspace(ctx context.Context, hint string) error {
	msg := fmt.Sprintf("%s not found. Copy the default appdir to get started? (idempotent)", hint)
	if !b.Choice(msg, true) {
		return fmt.Errorf("workspace not initialized (%s missing): run `resumegen init` to bootstrap one", hint)
	}
