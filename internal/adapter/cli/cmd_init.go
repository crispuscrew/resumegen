...
func (cmdInit) Run(ctx context.Context, deps Deps, args []string) error {
	...
	var (
		...
		fullExample = flags.Bool("full-example", false, "also extract the prompt templates for editing")
	...
	if *bare && *fullExample {
		return usageErr(errors.New("--bare and --full-example are mutually exclusive"))
	}
	if *bare && *withExample {
		return usageErr(errors.New("--bare and --with-example are mutually exclusive"))
	}
	...
	case 1:
		target = flags.Arg(0)
	default:
		return usageErr(fmt.Errorf("init takes at most one path argument; got %d", flags.NArg()))
	}
	...
	if *fullExample {
		subtrees = append(subtrees, "prompts")
	}

	extractor := appdir.NewSkeletonExtractor(deps.Skeleton)
	// The workspace needs its own config.toml (the loader requires one; the
	// README structure diagram promises one). Never overwrites an existing file.
	if copied, err := extractor.ExtractFile(ctx, "config.toml", filepath.Join(target, "config.toml")); err != nil {
		return fmt.Errorf("extract config.toml: %w", err)
	} else if copied {
		fmt.Println("  copied  config.toml")
	}
