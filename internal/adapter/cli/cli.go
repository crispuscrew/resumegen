...
type Deps struct {
	...
	// NewStore builds the application-tracker Store for a resolved appdir. It
	// is injected so tests can substitute an in-memory store; main leaves it
	// nil and the CLI falls back to the trackrepo filesystem store.
	NewStore func(appdir string) track.Store
}
...
// disabled when stdout is not a TTY or NO_COLOR is set. When stdin is not a
// terminal (script, pipe, CI) it declines without prompting: a headless first
// run must fail safe instead of silently writing a config tree into $HOME.
func UserChoice(msg string, defaultVal bool) bool {
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		fmt.Fprintf(os.Stderr, "%s: declined (stdin is not interactive)\n", msg)
		return false
	}
	var input string
