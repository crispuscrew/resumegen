...
const (
	...
	// data_gen.typ carries the full resume data: private by default, matching
	// the host renderer. README promises 0600 for the generated Typst source,
	// so use_container must not silently downgrade it.
...
func (r Renderer) Render(ctx context.Context, data domain.ResumeData, profile domain.Profile, cfg domain.Config) (string, float64, error) {
	...
	// Guard before MkdirAll: this path is also bind-mounted read-write (with an
	// SELinux relabel), so an unchecked "../" would both create and relabel a
	// directory outside the appdir.
	if err := host.EnsureContained(r.Appdir, hostOutPath); err != nil {
		return "", 0, err
	}
