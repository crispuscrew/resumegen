...
// TestSanitize_EscapesSlashCommentDelimiters pins the '/' escape. Typst treats
// "//" as a line comment and "/*...*/" as a block comment, so an unescaped
// slash in ordinary resume prose ("3/*4 of peak*/ sustained") silently deleted
// the text between the delimiters from the rendered PDF.
func TestSanitize_EscapesSlashCommentDelimiters(t *testing.T) {
	cases := []struct {
		name string
		in   string
	}{
		{"block comment swallows text", "Throughput 3/*4 of peak*/ sustained"},
		{"line comment", "Shipped 3 services // 2 of them in Go"},
		{"term list at line start", "/ Team lead for 6 engineers"},
		{"ordinary prose slash", "C/C++ and 80 MB/s"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := Sanitize(tc.in, Strict)
			if err != nil {
				t.Fatalf("Sanitize(%q) = error %v, want success", tc.in, err)
			}
			// Every slash must survive, and every one must be backslash-escaped
			// so no comment delimiter is left live. Counting `\/` against `/`
			// proves both at once: an unescaped slash makes the counts differ.
			if want, have := strings.Count(tc.in, "/"), strings.Count(got, `\/`); want != have {
				t.Errorf("Sanitize(%q) = %q: %d escaped slashes, want %d (an unescaped / is a live comment delimiter)", tc.in, got, have, want)
			}
		})
	}
}
